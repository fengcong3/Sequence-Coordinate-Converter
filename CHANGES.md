...
## 2023年12月更新 - 修复共享内存中64位指针和偏移量不一致问题
...
## 2024年1月更新 - 重大优化：按需访问模式减少内存占用

**主要改进：**

1. **优化大型索引文件的处理效率**：
   - 实现"按需访问"模式，避免为每个客户端复制整个records数组
   - 共享内存中只存储偏移量数据，客户端按需动态解析
   - 大幅降低了客户端内存占用，从与索引大小成正比减少到常量级内存消耗

2. **轻量级客户端上下文**：
   - 引入client_context_t结构，只存储必要的元数据和指针
   - 通过get_record_readname函数动态获取记录名称
   - 采用二分查找算法确保高效的记录定位

3. **兼容性与稳定性**：
   - 保持与现有API的兼容性，自动检测访问模式
   - 增强验证机制确保记录访问的可靠性
   - 支持新旧两种模式并行使用，平滑过渡

这次优化针对的是大规模索引文件（如10GB+）的使用场景。在以前的实现中，每个客户端都需要复制整个records数组，导致大量内存消耗。新的实现可以将客户端内存占用减少到几兆字节级别，同时不影响查询性能，这对于处理大型基因组数据特别重要。

目标场景下，此优化可以将15亿条记录的索引处理中，将客户端内存消耗从几十GB降低到几MB，是一个量级的改进，同时通过延迟加载策略保持了系统的响应性能。
